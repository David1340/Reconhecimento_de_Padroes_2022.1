...
dados = readdlm("bases/base_brinquedo.txt");
...
subplot(211)
scatter(x1[:,1],x1[:,2],color = "yellow");
scatter(x2[:,1],x2[:,2],color = "red");
scatter(x3[:,1],x3[:,2],color = "blue");
...
while(j < size(y_new,2))
    global j
    y_new[j+1]  = argmin(sum((yc[:,j+1] .- Matrix(I,3,3)).^2,dims = 1))[2]
